...
def _diff_search_matches(haystack, term, match_case, whole_word):
    """Replicates _search_matches() as a plain-python function so the diff-search
    worker thread can match text without touching any Qt widgets."""
    if not term:
        return False
...
class CommitListWidget(QListWidget):
    ...
    def _handle_multi_drag_drop(self, event):
        """Handles dragging a contiguous block of checked commits to a new position.

        Only moves the block when the checked commits form one adjacent (contiguous)
        group; otherwise the drop is rejected."""
        # Collect the checked indices (the block being moved)
        ...
        # The dragged row must be part of the checked block
        dragged_row = self.currentRow()
        if dragged_row not in checked:
            event.ignore()
            return

        # Contiguity check: the checked commits must be one adjacent group
        ...
        # Determine the insertion position from the drop location
        target_row = self.indexAt(event.position().toPoint()).row()
        if target_row == -1:
            insert_pos = len(remaining)  # dropped below the list -> append at end
        elif start <= target_row <= end:
            insert_pos = None  # dropped onto the block itself -> no move
        ...
        if new_order == list(range(count)):
            event.ignore()  # no actual reorder
            return
        ...
        # Visually reorder the existing items (preserves check states & item data)
