...
class RescanMixin:
    ...
    def _load_stats_async(self):
        ...
        def worker():
            try:
                ...
                _log(f"[perf] stats load (background): {time.monotonic()-_t:.3f}s")
        ...
        if stats:
            ...
            for i in range(self.list_widget.count()):
                ...
                if sha in stats:
                    ...
                    count += 1
            _log(f"[perf] _poll_stats apply ({count} commits): {time.monotonic()-_t:.3f}s")
    ...
    def load_browse_history_async(self):
        ...
        _log(f"[debug] load_browse_history_async: clearing post_action (was={getattr(self, '_browse_load_post_action', None)})")
    ...
    def _start_browse_load_poll(self):
        ...
        self._refresh_history_load()
        _log(f"[debug] _start_browse_load_poll: post_action={self._browse_load_post_action}")
