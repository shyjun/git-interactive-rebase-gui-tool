...
from lib.app_window.workers import NumstatWorker
...
class DiffMixin:
    ...
    def _on_numstat_ready(self, commit_sha, file_stats):
        ...
        if current_item:
            current_sha = current_item.text().split()[0]
            if current_sha != commit_sha:
                _log(f"[diff] numstat stale (current={current_sha[:11]}), ignoring")
                return
        # Cache the stats
        cache_entry = self.commit_cache.get(commit_sha, {})
