...
class BlameDialog(QDialog):
    ...
    def _open_diff_at_ref(self, filepath, sha):
        ...
        from lib.git_helpers import get_head_sha, run_difftool_temp_files, run_difftool_direct
        head_sha = get_head_sha(self.repo_path)
        dialog = DiffFileAtRefDialog(self.repo_path, filepath, sha, head_sha, parent=self)
        ...
        if dialog.use_direct:
            ...
            source_sha = head_sha if dialog.src_head_radio.isChecked() else sha
            print(f"[blame] Running temp: difftool {source_sha[:8]} {ref_sha[:8]} -- {target_file}")
