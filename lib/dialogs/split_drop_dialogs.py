...
class DropFileFromCommitDialog(QDialog):
    """Dialog for dropping file(s) changes from a commit.

    Provides filewise and treewise tabs with checkboxes, diff preview,
    and bidirectional sync — mirroring the SplitCommitDialog UX."""
    def __init__(self, repo_path, sha, files, font_size=10, font_family=None, parent=None):
        super().__init__(parent)
        self.repo_path = repo_path
        self.sha = sha
        self.font_size = font_size
        self.font_family = font_family
        self.setWindowTitle(f"Drop File(s) From Commit: {sha}")
        self.setMinimumSize(900, 680)
    ...
    def _update_drop_button(self):
        self.drop_btn.setEnabled(len(self._checked_filewise_files()) > 0)
    ...
    def show_filewise_context_menu(self, pos):
        ...
        if is_editable_branch(self):
            ...
            drop_action.triggered.connect(lambda checked=False, text=filepath: self._single_drop(text))
            menu.addAction(drop_action)

            remove_onwards_action = QAction("Remove file from this commit onwards", self)
            remove_onwards_action.triggered.connect(lambda checked=False, text=filepath: self.remove_file_onwards(text))
    ...
    def _single_drop(self, filepath):
